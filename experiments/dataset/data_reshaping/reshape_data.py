...
def reshape_diagnoses_data() -> pl.DataFrame:
    ...
    icd_code_embedings = pl.read_csv(DATA_DIR_ROOT / 'icd-10-cm-2022-0010.csv')
    ...
    diagnoses_data = diagnoses_data.join(
        icd_code_embedings.select(pl.all().exclude('desc')),
...
def plot_admission_data(admission_data: pl.DataFrame) -> None:
    ...
    # sns.barplot(plot_data, color='#5975a4')
    # plt.xticks(np.arange(0, 12, 1), plot_data.columns)
...
def get_length_of_stay(subject_and_hadm_ids: pl.DataFrame) -> pl.DataFrame:
    ...
    admission_data = admission_data.with_columns(
        nights_of_stay_group=pl.when(pl.col('nights_of_stay') < 10).then(pl.col('nights_of_stay'))
        .when(pl.col('nights_of_stay') < 100).then(11)
        .otherwise(12)
    )
...
if __name__ == '__main__':
    ...
    all_data.write_csv(DATA_DIR_ROOT / 'mimic_data_processed.csv')
